...
var _ = Describe("[sig-olmv1][OCPFeatureGate:NewOLMWebhookProviderOpenshiftServiceCA] OLMv1 operator with webhooks",
	Ordered, Serial, func() {
		...
		BeforeEach(func(ctx SpecContext) {
			...
			// Note: {{ TEST-BUNDLE }} will be auto-filled after we get opName from NewCatalogAndClusterBundles
			replacements := map[string]string{
				"{{ NAMESPACE }}":       "", // Auto-filled by NewCatalogAndClusterBundles
				"{{ TEST-BUNDLE }}":     "", // Auto-filled by NewCatalogAndClusterBundles with opName
