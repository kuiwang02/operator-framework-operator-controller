...
//   - "{{ NAMESPACE }}" or "{{ NS }}" - will be filled with the generated namespace name
...
func NewCatalogAndClusterBundles(ctx SpecContext, replacements map[string]string,
	...
	fileCatalog, fileCleanup := createTempTarBall(replacements, getAssetNamesCatalog, getAssetCatalog)
	DeferCleanup(fileCleanup)
	...
	// Note: cleanup is registered inside createClusterCatalogWithCleanup, no need to register again here
	_ = createClusterCatalogWithCleanup(ccName, nsName)

	// using named returns
	return unique, nsName, ccName, opName
}

// createClusterCatalogWithCleanup creates a ClusterCatalog and registers cleanup internally.
// The cleanup is registered with DeferCleanup before waiting for the catalog to be serving,
// ensuring cleanup happens even if waitForClusterCatalogServing panics.
// Returns a dummy cleanup function for compatibility (already registered, safe to ignore).
func createClusterCatalogWithCleanup(name, namespace string) func() {
	...
	// Define cleanup function immediately after creation
	cleanup := func() {
		By(fmt.Sprintf("deleting ClusterCatalog %q", name))
		Expect(k8sClient.Delete(context.Background(), cc)).To(Succeed())
	}

	// Register cleanup with DeferCleanup immediately to prevent resource leak
	// This ensures cleanup runs even if waitForClusterCatalogServing panics
	DeferCleanup(cleanup)

	// Wait for catalog to be serving
	waitForClusterCatalogServing(ctx, cc.Name)

	// Return cleanup for compatibility with existing pattern
	return cleanup
}
