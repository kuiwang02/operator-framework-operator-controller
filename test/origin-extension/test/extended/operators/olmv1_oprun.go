...
import (
	...
	olmv1util "github.com/openshift/operator-framework-operator-controller/test/origin-extension/test/extended/operators/olmv1util"
	exutil "github.com/openshift/operator-framework-operator-controller/test/origin-extension/test/extended/util"
	"github.com/openshift/operator-framework-operator-controller/test/origin-extension/test/extended/util/architecture"
...
var _ = g.Describe("[sig-operators] OLM v1 oprun should", func() {
	...
	// since OCP4.19
	g.It("Author:jiazha-ROSA-OSD_CCS-ARO-NonHyperShiftHOST-High-80078-Downstream feature gate promotion mechanics", func() {
	...
	g.It("Author:jiazha-LEVEL0-ROSA-OSD_CCS-ARO-NonHyperShiftHOST-ConnectedOnly-Critical-80458-clustercatalog get x509 error since it cannot get the custom CA automatically [Serial]", func() {
	...
	// author: kuiwang@redhat.com
	g.It("Author:kuiwang-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-Medium-68936-cluster extension can not be installed with insufficient permission sa for operand", func() {
	...
	// author: kuiwang@redhat.com
	g.It("Author:kuiwang-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-Medium-68937-cluster extension can not be installed with insufficient permission sa for operand rbac object", func() {
	...
	// author: kuiwang@redhat.com
	g.It("Author:kuiwang-LEVEL0-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-OSD_CCS-Medium-75492-cluster extension can not be installed with wrong sa or insufficient permission sa", func() {
		exutil.SkipForSNOCluster(oc)
		var (
			caseID                       = "75492"
			ns                           = "ns-" + caseID
	...
	// author: kuiwang@redhat.com
	g.It("Author:kuiwang-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-Medium-81538-preflight check on permission on allns mode", func() {
		if !olmv1util.IsFeaturegateEnabled(oc, "NewOLMPreflightPermissionChecks") {
	...
	g.It("Author:kuiwang-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-Medium-81664-preflight check on permission on own ns mode", func() {
		if !olmv1util.IsFeaturegateEnabled(oc, "NewOLMPreflightPermissionChecks") ||
			!olmv1util.IsFeaturegateEnabled(oc, "NewOLMOwnSingleNamespace") {
			g.Skip("NewOLMPreflightPermissionChecks or SingleOwnNamespaceInstallSupport is not enable, so skip it")
		}
		exutil.SkipForSNOCluster(oc)
		var (
			caseID                   = "81664"
		...
		start := "permissions to manage cluster extension:"
		end1 := "authorization evaluation error:"
		end2 := "for resolved bundle" // for load which does not include pR 1938
		filtered := olmv1util.FilterPermissions(output, start, end1, end2)
		e2e.Logf("===============================================================================")
		e2e.Logf("%v", filtered)
		e2e.Logf("===============================================================================")
		...
		exutil.By("create clusterrole")
		defer oc.AsAdmin().WithoutNamespace().Run("delete").Args("-f", clusterroleFile).Execute()
		err = oc.AsAdmin().WithoutNamespace().Run("apply").Args("-f", clusterroleFile).Execute()
		o.Expect(err).NotTo(o.HaveOccurred())
	...
	g.It("Author:kuiwang-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-Medium-81696-preflight check on permission on single ns mode", func() {
		if !olmv1util.IsFeaturegateEnabled(oc, "NewOLMPreflightPermissionChecks") ||
			!olmv1util.IsFeaturegateEnabled(oc, "NewOLMOwnSingleNamespace") {
			g.Skip("NewOLMPreflightPermissionChecks or SingleOwnNamespaceInstallSupport is not enable, so skip it")
		}
		...
		exutil.By("Create watch namespace")
		defer oc.WithoutNamespace().AsAdmin().Run("delete").Args("ns", nsWatch, "--ignore-not-found", "--force").Execute()
		...
		exutil.By("create role for ns")
		defer oc.AsAdmin().WithoutNamespace().Run("delete").Args("-f", roleNsFile).Execute()
		err = oc.AsAdmin().WithoutNamespace().Run("apply").Args("-f", roleNsFile).Execute()
		o.Expect(err).NotTo(o.HaveOccurred())

		exutil.By("create role for ns watch")
		defer oc.AsAdmin().WithoutNamespace().Run("delete").Args("-f", roleNsWatchFile).Execute()
	...
	// author: kuiwang@redhat.com
	g.It("Author:kuiwang-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-Medium-74618-ClusterExtension supports simple registry vzero bundles only", func() {
		exutil.SkipForSNOCluster(oc)
		var (
			ns                           = "ns-74618"
			sa                           = "sa74618"
	...
	// author: kuiwang@redhat.com
	g.It("Author:kuiwang-ROSA-OSD_CCS-ARO-NonHyperShiftHOST-Longduration-NonPreRelease-ConnectedOnly-Medium-76843-support disc with icsp [Disruptive]", func() {
		exutil.SkipForSNOCluster(oc)
		var (
			caseID                       = "76843"
			ns                           = "ns-" + caseID
	...
	// author: kuiwang@redhat.com
	g.It("Author:kuiwang-ROSA-OSD_CCS-ARO-NonHyperShiftHOST-Longduration-NonPreRelease-ConnectedOnly-Medium-76844-support disc with itms and idms [Disruptive]", func() {
	...
	// author: kuiwang@redhat.com
	g.It("Author:kuiwang-ROSA-OSD_CCS-ARO-NonHyperShiftHOST-Longduration-NonPreRelease-ConnectedOnly-Medium-78193-Runtime validation of container images using sigstore signatures [Disruptive]", func() {
		if !exutil.CheckAppearance(oc, 1*time.Second, 1*time.Second, exutil.Immediately,
	...
	// author: kuiwang@redhat.com
	g.It("Author:kuiwang-ROSA-OSD_CCS-ARO-NonHyperShiftHOST-Longduration-NonPreRelease-ConnectedOnly-Medium-78300-validation of container images using sigstore signatures with different policy [Disruptive]", func() {
		if !exutil.CheckAppearance(oc, 1*time.Second, 1*time.Second, exutil.Immediately,
			exutil.AsAdmin, exutil.WithoutNamespace, exutil.Appear, "crd", "clusterimagepolicies.config.openshift.io") {
			g.Skip("skip it because there is no ClusterImagePolicy")
		}
	...
	// author: kuiwang@redhat.com
	g.It("Author:kuiwang-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-Medium-76983-install index and bundle from private image", func() {
		exutil.SkipForSNOCluster(oc)
	...
	// author: kuiwang@redhat.com
	g.It("Author:kuiwang-ROSA-OSD_CCS-ARO-NonHyperShiftHOST-Longduration-NonPreRelease-ConnectedOnly-Medium-76985-authfile is updated automatically [Disruptive].", func() {
		exutil.SkipForSNOCluster(oc)
		var (
			caseID = "76985"
		)
	...
	g.It("Author:xzha-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-OSD_CCS-High-68821-olmv1 Supports Version Ranges during Installation", func() {
		var (
			baseDir                                       = exutil.FixturePath("testdata", "olm", "v1")
			clustercatalogTemplate                        = filepath.Join(baseDir, "clustercatalog.yaml")
			clusterextensionTemplate                      = filepath.Join(baseDir, "clusterextension.yaml")
	...
	// author: xzha@redhat.com
	g.It("Author:xzha-LEVEL0-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-OSD_CCS-Medium-69196-olmv1 Supports Version Ranges during clusterextension upgrade", func() {
		var (
			baseDir                      = exutil.FixturePath("testdata", "olm", "v1")
			clustercatalogTemplate       = filepath.Join(baseDir, "clustercatalog.yaml")
			clusterextensionTemplate     = filepath.Join(baseDir, "clusterextension.yaml")
	...
	// author: xzha@redhat.com
	g.It("Author:xzha-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-OSD_CCS-High-74108-olm v1 supports legacy upgrade edges", func() {
		var (
			baseDir                      = exutil.FixturePath("testdata", "olm", "v1")
			clustercatalogTemplate       = filepath.Join(baseDir, "clustercatalog.yaml")
	...
	// author: xzha@redhat.com
	g.It("Author:xzha-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-OSD_CCS-Medium-74923-no two ClusterExtensions can manage the same underlying object", func() {
		var (
			baseDir                      = exutil.FixturePath("testdata", "olm", "v1")
			clustercatalogTemplate       = filepath.Join(baseDir, "clustercatalog.yaml")
	...
	// author: xzha@redhat.com
	g.It("Author:xzha-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-Medium-75501-the updates of various status fields is orthogonal", func() {
		var (
			baseDir                      = exutil.FixturePath("testdata", "olm", "v1")
			clustercatalogTemplate       = filepath.Join(baseDir, "clustercatalog.yaml")
			clusterextensionTemplate     = filepath.Join(baseDir, "clusterextension.yaml")
	...
	// author: xzha@redhat.com
	g.It("Author:xzha-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-OSD_CCS-High-76685-olm v1 supports selecting catalogs", func() {
		var (
			baseDir                                  = exutil.FixturePath("testdata", "olm", "v1")
			clustercatalogTemplate                   = filepath.Join(baseDir, "clustercatalog-withlabel.yaml")
			clusterextensionTemplate                 = filepath.Join(baseDir, "clusterextensionWithoutChannelVersion.yaml")
	...
	// author: xzha@redhat.com
	g.It("Author:xzha-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-OSD_CCS-Medium-77972-olm v1 Supports MaxOCPVersion in properties file", func() {
		var (
			baseDir                  = exutil.FixturePath("testdata", "olm", "v1")
			clustercatalogTemplate   = filepath.Join(baseDir, "clustercatalog.yaml")
		...
		exutil.By("3) upgrade clusterextension to 1.1.0, olm.maxOpenShiftVersion is 4.19")
		...
		exutil.By("4) upgrade clusterextension to 1.2.0, olm.maxOpenShiftVersion is 4.20")
	...
	// author: xzha@redhat.com
	g.It("Author:xzha-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-High-78393-support metrics", func() {
		...
		errWait := wait.PollUntilContextTimeout(context.TODO(), 10*time.Second, 30*time.Second, false, func(ctx context.Context) (bool, error) {
			metricsMsg, err := oc.AsAdmin().NotShowInfo().WithoutNamespace().Run("exec").Args("-n", "openshift-operator-lifecycle-manager", catalogPodname, "-i", "--", "curl", "-k", "-H", fmt.Sprintf("Authorization: Bearer %v", metricsToken), queryContent).Output()
			e2e.Logf("err:%v", err)
		...
		metricsMsg, _ = oc.AsAdmin().NotShowInfo().WithoutNamespace().Run("exec").Args("-n", "openshift-operator-lifecycle-manager", catalogPodname, "-i", "--", "curl", "-k", "-H", fmt.Sprintf("Authorization: Bearer %v", wrongToken), queryContent).Output()
	...
	g.It("Author:xzha-LEVEL0-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-High-79770-metrics are collected by default", func() {
	...
	g.It("Author:xzha-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-High-80117-Single Namespace Install Mode should be supported	", func() {
		...
		var (
			baseDir                           = exutil.FixturePath("testdata", "olm", "v1")
			clustercatalogTemplate            = filepath.Join(baseDir, "clustercatalog.yaml")
			clusterextensionOwnSingleTemplate = filepath.Join(baseDir, "clusterextension-withoutChannel-OwnSingle.yaml")
			clusterextensionTemplate          = filepath.Join(baseDir, "clusterextensionWithoutChannel.yaml")
		...
		exutil.By("1) Create namespace, sa, clustercatalog")
		defer oc.WithoutNamespace().AsAdmin().Run("delete").Args("ns", ns, "--ignore-not-found").Execute()
		err := oc.WithoutNamespace().AsAdmin().Run("create").Args("ns", ns).Execute()
		o.Expect(err).NotTo(o.HaveOccurred())
		o.Expect(olmv1util.Appearance(oc, exutil.Appear, "ns", ns)).To(o.BeTrue())
		...
		exutil.By("2) install clusterextension, version 1.0.1, without creating watch ns")
		defer clusterextension.Delete(oc)
		clusterextension.CreateWithoutCheck(oc)
		errWait := wait.PollUntilContextTimeout(context.TODO(), 3*time.Second, 150*time.Second, false, func(ctx context.Context) (bool, error) {
			message, _ := olmv1util.GetNoEmpty(oc, "clusterextension", clusterextension.Name, "-o", "jsonpath={.status.conditions[*].message}")
			if !strings.Contains(message, "failed to create resource: namespaces") {
				e2e.Logf("status is %s", message)
				return false, nil
			}
			return true, nil
		...
		exutil.By("3) create watch ns")
		defer oc.WithoutNamespace().AsAdmin().Run("delete").Args("ns", nsWatch, "--ignore-not-found").Execute()
		err = oc.WithoutNamespace().AsAdmin().Run("create").Args("ns", nsWatch).Execute()
		o.Expect(err).NotTo(o.HaveOccurred())
		o.Expect(olmv1util.Appearance(oc, exutil.Appear, "ns", nsWatch)).To(o.BeTrue())

		exutil.By("4) create clusterextension, version 1.0.1")
		clusterextension.Create(oc)
		o.Expect(clusterextension.InstalledBundle).To(o.ContainSubstring("v1.0.1"))

		exutil.By("4.1) check deployment")
		...
		exutil.By("4.2) check rolebinding")
		...
		exutil.By("5) upgrade clusterextension to 1.0.2, v1.0.2 only support singleNamespace")
		...
		exutil.By("5.1) check deployment")
		deploymentTargetNS, _ = olmv1util.GetNoEmpty(oc, "deployment", "nginx80117-controller-manager", "-n", ns, "-o", `jsonpath={.spec.template.metadata.annotations.olm\.targetNamespaces}`)
		o.Expect(deploymentTargetNS).To(o.ContainSubstring(nsWatch))
		exutil.By("5.2) check rolebinding")
		rdNS, _ = olmv1util.GetNoEmpty(oc, "rolebinding", "-l", "olm.operatorframework.io/owner-name="+clusterextension.Name, "-n", nsWatch, "-o", `jsonpath={..subjects[].namespace}`)
		o.Expect(rdNS).To(o.ContainSubstring(ns))

		exutil.By("6) upgrade clusterextension to 1.1.0, support allnamespace")
		...
		exutil.By("6.1) check deployment")
		...
		exutil.By("6.2) check rolebinding")
		...
		exutil.By("7) upgrade clusterextension to 2.0.0, support singleNamespace")
		clusterextension.Version = "2.0.0"
		clusterextension.Create(oc)
		errWait = wait.PollUntilContextTimeout(context.TODO(), 3*time.Second, 150*time.Second, false, func(ctx context.Context) (bool, error) {
			clusterextension.GetBundleResource(oc)
		...
		exutil.By("7.1) check deployment")
		deploymentTargetNS, _ = olmv1util.GetNoEmpty(oc, "deployment", "nginx80117-controller-manager", "-n", ns, "-o", `jsonpath={.spec.template.metadata.annotations.olm\.targetNamespaces}`)
		o.Expect(deploymentTargetNS).To(o.ContainSubstring(nsWatch))
		exutil.By("7.2) check rolebinding")
		rdNS, _ = olmv1util.GetNoEmpty(oc, "rolebinding", "-l", "olm.operatorframework.io/owner-name="+clusterextension.Name, "-n", nsWatch, "-o", `jsonpath={..subjects[].namespace}`)
		o.Expect(rdNS).To(o.ContainSubstring(ns))

		exutil.By("8) check not support install two same clusterextensions")
		ns2 := ns + "-2"
		nsWatch2 := nsWatch + "-2"
		sa2 := "sa80117-2"
		saCrb2 := olmv1util.SaCLusterRolebindingDescription{
		...
		o.Expect(err).NotTo(o.HaveOccurred())
		o.Expect(olmv1util.Appearance(oc, exutil.Appear, "ns", ns2)).To(o.BeTrue())
		defer oc.WithoutNamespace().AsAdmin().Run("delete").Args("ns", nsWatch2, "--ignore-not-found").Execute()
		...
		exutil.By("9) Test SUCCESS")

	})

	g.It("Author:xzha-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-High-80120-Own Namespace Install Mode should be supported	", func() {
		if !olmv1util.IsFeaturegateEnabled(oc, "NewOLMOwnSingleNamespace") {
			g.Skip("SingleOwnNamespaceInstallSupport is not enable, so skip it")
		}
		var (
		...
		exutil.By("2) install clusterextension, version 1.0.1")
		defer clusterextension.Delete(oc)
		clusterextension.Create(oc)
		o.Expect(clusterextension.InstalledBundle).To(o.ContainSubstring("v1.0.1"))
		exutil.By("2.1) check deployment")
		...
		exutil.By("2.2) check rolebinding")
		...
		exutil.By("3) upgrade clusterextension to 1.0.2, v1.0.2 only support OwnNamespace")
		clusterextension.Patch(oc, `{"spec":{"source":{"catalog":{"version":"1.0.2"}}}}`)
		errWait := wait.PollUntilContextTimeout(context.TODO(), 3*time.Second, 150*time.Second, false, func(ctx context.Context) (bool, error) {
			clusterextension.GetBundleResource(oc)
			if strings.Contains(clusterextension.InstalledBundle, "1.0.2") {
		...
		exutil.By("4) upgrade clusterextension to 3.0.0, support allnamespace")
		clusterextensionAllNs.Create(oc)
		errWait = wait.PollUntilContextTimeout(context.TODO(), 3*time.Second, 150*time.Second, false, func(ctx context.Context) (bool, error) {
			clusterextension.GetBundleResource(oc)
		...
		exutil.By("5) upgrade clusterextension to 4.0.0, support OwnNamespace")
		...
		exutil.By("6) if the annotations is not correct, error should be raised")
		clusterextension.Delete(oc)
		clusterextension = olmv1util.ClusterExtensionDescription{
			Name:             "clusterextension-80120",
			InstallNamespace: ns,
		...
		errWait = wait.PollUntilContextTimeout(context.TODO(), 3*time.Second, 150*time.Second, false, func(ctx context.Context) (bool, error) {
			message, _ := olmv1util.GetNoEmpty(oc, "clusterextension", clusterextension.Name, "-o", "jsonpath={.status.conditions[*].message}")
			if !strings.Contains(message, "supported install modes [OwnNamespace] do not support target namespaces") {
				e2e.Logf("status is %s", message)
				return false, nil
			}
			return true, nil
	...
	// author: bandrade@redhat.com
	g.It("Author:bandrade-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-High-70723-olmv1 downgrade version", func() {
		var (
			baseDir                      = exutil.FixturePath("testdata", "olm", "v1")
			clustercatalogTemplate       = filepath.Join(baseDir, "clustercatalog.yaml")
			clusterextensionTemplate     = filepath.Join(baseDir, "clusterextension.yaml")
	...
	// author: bandrade@redhat.com
	g.It("Author:bandrade-ROSA-OSD_CCS-ARO-NonHyperShiftHOST-Medium-75877-Make sure that rukpak is removed from payload", func() {
	...
	// author: bandrade@redhat.com
	g.It("Author:bandrade-LEVEL0-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-Medium-77413-Check if ClusterCatalog is in Serving properly", func() {
