...
import (
	...
	container "github.com/openshift/operator-framework-operator-controller/test/origin-extension/test/extended/util/container"
...
var _ = g.Describe("[sig-operators] OLM v1 opeco should", func() {
	...
	g.It("Author:xzha-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-VMonly-High-69758-Catalogd Polling remote registries for update to images content", func() {
		var (
	...
	// author: xzha@redhat.com
	g.It("Author:xzha-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-High-69242-Catalogd deprecated package/bundlemetadata/catalogmetadata from clustercatalog CR", func() {
	...
	// author: xzha@redhat.com
	g.It("Author:xzha-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-High-69069-Replace pod-based image unpacker with an image registry client", func() {
		var (
			baseDir                = exutil.FixturePath("testdata", "olm", "v1")
			clustercatalogTemplate = filepath.Join(baseDir, "clustercatalog.yaml")
			clustercatalog         = olmv1util.ClusterCatalogDescription{
	...
	// author: xzha@redhat.com
	g.It("Author:xzha-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-High-69869-Catalogd Add metrics to the Storage implementation", func() {
	...
	// author: xzha@redhat.com
	g.It("Author:xzha-LEVEL0-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-High-69202-Catalogd clustercatalog offer the operator content through http server off cluster", func() {
		exutil.SkipOnProxyCluster(oc)
		var (
			baseDir                = exutil.FixturePath("testdata", "olm", "v1")
			clustercatalogTemplate = filepath.Join(baseDir, "clustercatalog.yaml")
	...
	// author: xzha@redhat.com
	g.It("Author:xzha-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-High-73219-Fetch deprecation data from the catalogd http server", func() {
		var (
			baseDir                = exutil.FixturePath("testdata", "olm", "v1")
			clustercatalogTemplate = filepath.Join(baseDir, "clustercatalog.yaml")
			clustercatalog         = olmv1util.ClusterCatalogDescription{
	...
	// author: xzha@redhat.com
	g.It("Author:xzha-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-High-73289-Check the deprecation conditions and messages", func() {
		var (
			baseDir                      = exutil.FixturePath("testdata", "olm", "v1")
			clustercatalogTemplate       = filepath.Join(baseDir, "clustercatalog.yaml")
	...
	// author: xzha@redhat.com
	g.It("Author:xzha-LEVEL0-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-High-74948-catalog offer the operator content through https server", func() {
		var (
			baseDir                      = exutil.FixturePath("testdata", "olm", "v1")
			clustercatalogTemplate       = filepath.Join(baseDir, "clustercatalog.yaml")
			clusterextensionTemplate     = filepath.Join(baseDir, "clusterextension.yaml")
	...
	// author: xzha@redhat.com
	g.It("Author:xzha-LEVEL0-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-OSD_CCS-High-74978-CRD upgrade will be prevented if the Scope is switched between Namespaced and Cluster", func() {
		var (
			baseDir                      = exutil.FixturePath("testdata", "olm", "v1")
			clustercatalogTemplate       = filepath.Join(baseDir, "clustercatalog.yaml")
			clusterextensionTemplate     = filepath.Join(baseDir, "clusterextension.yaml")
	...
	// author: xzha@redhat.com
	g.It("Author:xzha-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-High-75218-Disabling the CRD Upgrade Safety preflight checks", func() {
		var (
			baseDir                      = exutil.FixturePath("testdata", "olm", "v1")
			clustercatalogTemplate       = filepath.Join(baseDir, "clustercatalog.yaml")
			clusterextensionTemplate     = filepath.Join(baseDir, "clusterextension.yaml")
	...
	// author: xzha@redhat.com
	g.It("Author:xzha-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-High-75122-CRD upgrade check Removing an existing stored version and add a new CRD with no modifications to existing versions", func() {
		...
		clusterextension.CheckClusterExtensionCondition(oc, "Progressing", "message",
			`failed: stored version "v1alpha1" removed`, 10, 60, 0)
	...
	g.It("Author:xzha-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-High-75123-High-75217-CRD upgrade checks for changes in required field and field type", func() {
		exutil.SkipForSNOCluster(oc)
		var (
			caseID                       = "75123"
			labelValue                   = caseID
		...
		clusterextension.CheckClusterExtensionCondition(oc, "Progressing", "message",
			`new required fields [requiredfield2] added`, 10, 60, 0)
		...
		clusterextension.CheckClusterExtensionCondition(oc, "Progressing", "message",
			`field "^.spec.field": type changed from "integer" to "string"`, 10, 60, 0)
	...
	// author: xzha@redhat.com
	g.It("Author:xzha-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-High-75124-CRD upgrade checks for changes in default values", func() {
		exutil.SkipForSNOCluster(oc)
		var (
			caseID                       = "75124"
			labelValue                   = caseID
		...
		clusterextension.CheckClusterExtensionCondition(oc, "Progressing", "message",
			`default value "\"default-string-jitli\"" added when there was no default previously`, 10, 60, 0)
		...
		clusterextension.CheckClusterExtensionCondition(oc, "Progressing", "message",
			`failed: version "v1alpha1", field "^.spec.defaultenum": default value changed from "\"value1\"" to "\"value3\""`, 10, 60, 0)
		...
		clusterextension.CheckClusterExtensionCondition(oc, "Progressing", "message",
			`field "^.spec.defaultint": default value "9" removed`, 10, 60, 0)

	})

	// author: xzha@redhat.com
	g.It("Author:xzha-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-High-75515-CRD upgrade checks for changes in enumeration values", func() {
		exutil.SkipForSNOCluster(oc)
		var (
			caseID                       = "75515"
			labelValue                   = caseID
		...
		o.Expect(err).NotTo(o.HaveOccurred())
		clusterextension.CheckClusterExtensionCondition(oc, "Progressing", "message",
			`field "^.spec.unenumfield": enum constraints ["value1" "value2" "value3"] added when there were no restrictions previously`, 10, 60, 0)
		o.Expect(clusterextension.InstalledBundle).To(o.ContainSubstring("v1.0.1"))
		clusterextension.Delete(oc)
	...
	// author: xzha@redhat.com
	g.It("Author:xzha-LEVEL0-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-High-75516-CRD upgrade checks for the field maximum minimum changes", func() {
		exutil.SkipForSNOCluster(oc)
		var (
			caseID                       = "75516"
			labelValue                   = caseID
	...
	// author: xzha@redhat.com
	g.It("Author:xzha-ROSA-OSD_CCS-ARO-ConnectedOnly-NonHyperShiftHOST-Critical-75441-Catalogd supports compression and jsonlines format", func() {
		var (
			baseDir                = exutil.FixturePath("testdata", "olm", "v1")
			clustercatalogTemplate = filepath.Join(baseDir, "clustercatalog.yaml")
			clustercatalog         = olmv1util.ClusterCatalogDescription{
